...
// Incidents data definition is now managed via component state
...
const IncomingIncidents: React.FC = () => {
  ...
  const [selected, setSelected] = useState<typeof incidentsList[0] | null>(null);

  React.useEffect(() => {
    if (incidentsList.length > 0 && !selected) {
      setSelected(incidentsList[0]);
    }
  }, [incidentsList, selected]);

  const handleDispatch = (id: string) => {
    setIncidentsList(prev => prev.map(inc => inc.id === id ? { ...inc, status: 'dispatched' } : inc));
    setSelected(prev => prev && prev.id === id ? { ...prev, status: 'dispatched' } : prev);
    toast.success(`Fire units dispatched to incident ${id}!`);
  };

  const filtered = filter === 'all' ? incidentsList : incidentsList.filter(i => i.status === filter || i.priority === filter);
    ...
    <RoleDashboardLayout title="Incoming Incidents">
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { key: 'pending', label: 'Pending Dispatch', count: incidentsList.filter(i => i.status === 'pending').length, color: 'text-red-600 bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' },
            { key: 'dispatched', label: 'Units Dispatched', count: incidentsList.filter(i => i.status === 'dispatched').length, color: 'text-blue-600 bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' },
            { key: 'contained', label: 'Contained', count: incidentsList.filter(i => i.status === 'contained').length, color: 'text-orange-600 bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800' },
            { key: 'resolved', label: 'Resolved Today', count: incidentsList.filter(i => i.status === 'resolved').length, color: 'text-green-600 bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' },
          ].map(s => (
            ...
            {filtered.map(inc => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {inc.status === 'pending' ? (
                    <Button size="sm" className="gradient-fire text-white border-0 text-xs flex-shrink-0 font-semibold" onClick={e => { e.stopPropagation(); handleDispatch(inc.id); }}>
            ...
            <div className="bg-card border border-border rounded-2xl p-5">
              ...
              <div className="mt-4 flex gap-2">
                {selected.status === 'pending' ? (
                  <Button size="sm" className="flex-1 gradient-fire text-white border-0 text-xs font-semibold hover:opacity-90" onClick={() => handleDispatch(selected.id)}>Dispatch Unit</Button>
                ) : (
                  <div className="flex-1 text-center py-2 rounded-lg bg-green-500/10 text-green-600 dark:text-green-400 font-semibold text-xs border border-green-500/20">Units responding</div>
                )}
